...
function message_use() {
    printf "%b\n" \
	   "`colour lb $(basename $0)` copies a file/directory from `colour lb csuser` in an Amazon Machine Image (AMI) to the local machine." \
	   " " \
	   "usage: " \
	   " " \
...
case $# in
	...
	if [ -f $loginKeyFile ]; then
		...
		if ssh -i "$loginKeyFile" "$user@$machine.$domain" "test -f $remoteFileDir" ; then # idem
			...
			.*|..*|~/*) # copy the remote directory entry / not only the file within
			    if [ $verboseFlag == TRUE ]; then
